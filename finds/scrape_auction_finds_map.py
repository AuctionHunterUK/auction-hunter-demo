...
<head>
  ...
  <style>
    ...
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    nav.jump .wanted-filter-button.active {{ color: var(--accent); background: transparent; }}
    nav.jump .wanted-filter-button.active .jump-count {{ color: var(--accent); }}
    nav.jump .wanted-filter-button.active .refined-switch {{ background: var(--accent); border-color: var(--accent); }}
    nav.jump .wanted-filter-button.active .refined-switch-knob {{ transform: translateY(-14px); }}
    nav.jump .wanted-filter-button.active .refined-switch-state {{ color: var(--accent); }}
    @media (hover: hover) and (pointer: fine) {{
      nav.jump .wanted-filter-button:hover {{ color: var(--ink); }}
      nav.jump .wanted-filter-button.active:hover {{ color: var(--accent); background: transparent; }}
    }}
    .new-badge {{
    ...
    .lot-map-card {{
      ...
      border-radius: 9px;
      box-shadow: 0 5px 18px rgba(30,40,32,.18);
      padding: 10px 12px;
      color: var(--ink);
      font-family: 'DM Sans', sans-serif;
      line-height: 1.35;
    }}
    .lot-map-card.visible {{ display: block; }}
    .lot-map-card-name {{
      font-family: 'Playfair Display', serif;
      font-size: .88rem;
      font-weight: 700;
    ...
    .lot-reminder-trigger {{
      position: absolute; top: 8px; right: 8px; z-index: 4;
      display: flex; align-items: center; justify-content: center;
      width: 32px; height: 32px; padding: 0;
      border: 1px solid #bdbdbd; border-radius: 50%;
      background: #fff; color: #111;
      font: 400 1rem/1 'DM Sans', sans-serif;
      cursor: pointer; box-shadow: 0 2px 7px rgba(20,30,40,.12);
      transition: background .15s ease, border-color .15s ease, transform .15s ease;
      -webkit-tap-highlight-color: transparent;
    }}
    .lot-reminder-trigger:hover,
    .lot-reminder-trigger:focus-visible {{
      background: #fff; border-color: #111; transform: translateY(-1px);
      outline: none;
    }}
    .lot-reminder-popup {{
      position: fixed; z-index: 1650; display: none;
      ...
      padding: 13px 15px; color: var(--ink); font-size: .74rem; line-height: 1.45;
    }}
    .lot-reminder-popup.visible {{ display: block; }}
    .lot-reminder-popup-title {{
      padding-right: 24px; font-family: 'Playfair Display', serif;
      font-size: .92rem; font-weight: 700; line-height: 1.3;
    }}
    .lot-reminder-popup-copy {{ margin-top: 6px; color: var(--muted); }}
    .lot-reminder-popup-note {{ margin-top: 8px; color: var(--accent); font-size: .68rem; font-weight: 700; }}
    .lot-reminder-close {{ display: none; }}
    .lot-preview-popup {{
    ...
    .lot-preview-close {{ display: none; }}
    ...
    @media (hover: none), (pointer: coarse) {{
      .lot-preview-trigger {{
        position: absolute; right: 46px; top: 8px; z-index: 3; display: inline-block;
      ...
      .lot-preview-popup-title {{ padding-right: 34px; }}
      .lot-preview-close {{
        ...
        background: transparent; color: var(--muted); font: 400 1.45rem/1 'DM Sans', sans-serif;
        cursor: pointer; -webkit-tap-highlight-color: transparent;
      }}
      .lot-preview-close:active {{ background: var(--accent-soft); color: var(--accent); }}
      .lot-reminder-close {{
        position: absolute; top: 5px; right: 5px; display: flex;
        align-items: center; justify-content: center;
        width: 32px; height: 32px; padding: 0; border: 0; border-radius: 50%;
        background: transparent; color: var(--muted); font: 400 1.35rem/1 'DM Sans', sans-serif;
        cursor: pointer;
      }}
      .lot-reminder-close:active {{ background: var(--accent-soft); color: var(--accent); }}
    }}
    ...
    .pin-highlighted {{
      ...
      border-radius: 50%;
      box-shadow: 0 0 12px rgba(31,42,33,.55);
    }}
    ...
    @media (max-width: 800px) {{
      ...
      .hrow2 {{ grid-template-columns: 1fr; row-gap: 4px; }}
      .header-utility-nav {{ grid-row: 1; justify-self: end; }}
      .header-page-tools {{ grid-row: 2; width: 100%; }}
      .jump {{ max-width: 100%; }}
      ...
      nav.jump .wanted-filter-button.active {{ padding-left: 9px; }}
    ...
    /* Finds header proposal: search first, then prominent catalogue controls. */
    @media (min-width: 801px) {{
      header {{ padding: 0 24px; }}
      .headtop {{
        min-height: 76px;
        grid-template-columns: max-content minmax(160px, 620px) minmax(0, 1fr) max-content;
        column-gap: 24px;
      }}
      .brand h1 {{ font-size: 1.5rem; }}
      .search-box input {{ min-height: 44px; border-radius: 2px; border-color: #d6d6d6; font-size: 1rem; padding-left: 14px; }}
      .search-box input:focus {{ border-color: var(--accent); box-shadow: 0 0 0 2px var(--accent-soft); }}
      .search-box .clear-btn {{ width: 26px; height: 26px; }}
      .featured-searches-trigger {{ font-size: .875rem; padding: 10px 0; border-bottom-color: #c6cec6; }}
      .app-nav {{ border-radius: 22px; padding: 3px; background: rgba(58,92,59,.1); border-color: var(--accent); }}
      .app-nav-link {{ border-radius: 18px; font-size: .82rem; padding: 7px 18px; }}
      .app-nav-link.on {{ box-shadow: 0 1px 4px rgba(58,92,59,.35); }}
      .hrow2 {{ min-height: 64px; grid-template-columns: minmax(0, 1fr) max-content minmax(0, 1fr); border-top: 1px solid #ececec; column-gap: 28px; }}
      .header-page-tools {{ grid-column: 2; justify-self: center; display: flex; align-items: center; justify-content: center; gap: 18px; }}
      header .tagline {{ display: none; }}
      #searchResults {{ order: 2; font-size: .8125rem; white-space: nowrap; }}
      #searchResults:empty {{ display: none; }}
      header nav.jump {{ gap: 12px; min-width: 0; }}
      nav.jump .group-tab {{ min-height: 48px; font-size: 1rem; font-weight: 600; padding: 10px 14px; color: #555; }}
      nav.jump .group-tab:first-child {{ padding-left: 0; }}
      nav.jump .group-tab.active {{ color: var(--accent); }}
      nav.jump .group-tab.active::after {{ bottom: 0; height: 3px; }}
      nav.jump .group-tab .jump-count,
      nav.jump .wanted-filter-button .jump-count {{
        display: inline-block; min-width: 26px; padding: 2px 6px; margin-left: 5px;
        background: #f1f1f1; border-radius: 2px; color: #626262; font-size: .8125rem; font-weight: 500;
      }}
      nav.jump .group-tab.active .jump-count {{ background: var(--accent-soft); color: var(--accent); }}
      nav.jump .wanted-filter-button {{ min-height: 42px; font-size: .9375rem; font-weight: 600; padding: 6px 0 6px 24px; color: #333; }}
      nav.jump .wanted-filter-button.active {{ padding: 6px 0 6px 24px; color: var(--accent); }}
      nav.jump .wanted-filter-button.active .jump-count {{ color: var(--accent); background: var(--accent-soft); }}
      .header-utility-nav {{ grid-column: 3; justify-self: end; gap: 24px; }}
      .header-utility-nav a {{ display: inline-flex; align-items: center; min-height: 44px; font-size: .9375rem; color: #555; }}
      header button:focus-visible, header a:focus-visible {{ outline: 2px solid var(--accent); outline-offset: 3px; }}
      .featured-searches-popover, .refined-search-popover {{ border-radius: 2px; }}
      .featured-searches-popover p, .refined-search-popover p,
      .featured-searches-popover li, .refined-search-popover li,
      .refined-search-popover a {{ font-size: .8125rem; }}
    }}
    @media (min-width: 801px) and (max-width: 1100px) {{
      .headtop {{ column-gap: 16px; grid-template-columns: max-content minmax(120px, 1fr) max-content max-content; }}
      .brand h1 {{ font-size: 1.25rem; }}
      .app-nav-link {{ padding-left: 14px; padding-right: 14px; }}
      .hrow2 {{ display: flex; flex-wrap: wrap; column-gap: 20px; }}
      .header-page-tools {{ flex: 1 1 auto; flex-wrap: wrap; gap: 0 14px; }}
      .header-utility-nav {{ margin-left: auto; gap: 18px; }}
      nav.jump .group-tab {{ padding-left: 10px; padding-right: 10px; }}
    }}
...
<button aria-controls="featuredSearchesPopover" aria-expanded="false" aria-haspopup="true" class="featured-searches-trigger" id="featuredSearchesTrigger" type="button">Daily Search Terms <span class="featured-searches-count">{featured_term_count}</span></button>
<div aria-label="Current Daily Search Terms" class="featured-searches-popover" id="featuredSearchesPopover" role="dialog">
<strong>Current Daily Search Terms</strong>
...
<p>These active phrases narrow the lots already selected by your Daily Search Terms.</p>
...
{house_popup_js}
    ...
    (function initLotPreview() {{
      ...
      function showPreview(titleEl, trigger) {{
        ...
        closeButton.className = 'lot-preview-close';
        closeButton.setAttribute('aria-label', 'Close lot details');
    ...
    // ── PLANNED AUCTION REMINDER (demonstration only) ──
    (function initPlannedReminders() {{
      const popup = document.createElement('div');
      popup.className = 'lot-reminder-popup';
      popup.id = 'lot-reminder-popup';
      popup.setAttribute('role', 'dialog');
      popup.setAttribute('aria-label', 'Planned auction reminder');
      popup.setAttribute('aria-hidden', 'true');
      document.body.appendChild(popup);
      ...
      function appendText(className, value, tagName = 'div') {{
        const el = document.createElement(tagName);
        el.className = className;
        el.textContent = value;
      ...
      function positionPopup(anchor) {{
        ...
        const popupRect = popup.getBoundingClientRect();
        const edge = 14;
        let left = Math.min(Math.max(edge, rect.right - popupRect.width), window.innerWidth - popupRect.width - edge);
        let top = rect.bottom + 8;
        if (top + popupRect.height > window.innerHeight - edge) top = rect.top - popupRect.height - 8;
        popup.style.left = `${{Math.max(edge, left)}}px`;
        popup.style.top = `${{Math.max(edge, top)}}px`;
      ...
      function showPopup(trigger, pin = false) {{
        clearTimeout(showTimer);
        ...
        const closeButton = document.createElement('button');
        closeButton.type = 'button';
        closeButton.className = 'lot-reminder-close';
        closeButton.setAttribute('aria-label', 'Close reminder information');
        closeButton.textContent = '×';
        closeButton.addEventListener('click', event => {{
          event.preventDefault();
          event.stopPropagation();
        ...
        appendText(
          'lot-reminder-popup-copy',
          'The email could include the official EasyLive lot link, with a future option for a second reminder shortly before the auction begins. Live lot-position alerts would require real-time EasyLive integration.'
        );
        appendText('lot-reminder-popup-note', 'Demonstration only — no reminder has been created.');
        popup.style.visibility = 'hidden';
        popup.classList.add('visible');
        popup.setAttribute('aria-hidden', 'false');
      ...
      document.querySelectorAll('.card-shell').forEach(shell => {{
        const title = (shell.querySelector('.lot-title-text')?.textContent || 'this lot').trim();
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'lot-reminder-trigger';
        button.setAttribute('aria-label', 'Planned reminder for: ' + title);
        button.setAttribute('aria-controls', popup.id);
        button.setAttribute('aria-expanded', 'false');
        button.title = 'Planned auction email reminder';
        button.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/><path d="M10 21h4M12 2V1"/></svg>';
        shell.appendChild(button);

        if (hoverCapable) {{
          button.addEventListener('mouseenter', () => {{
            showTimer = window.setTimeout(() => showPopup(button), 180);
          }});
          button.addEventListener('mouseleave', () => {{
            clearTimeout(showTimer);
            if (!pinnedOpen) hidePopup();
          }});
          button.addEventListener('focus', () => showPopup(button));
        ...
        button.addEventListener('click', event => {{
          event.preventDefault();
          event.stopPropagation();
          if (activeTrigger === button && pinnedOpen) hidePopup();
          else showPopup(button, true);
        }});
    ...
    function applyLotFilters() {{
      ...
      const switchState = document.getElementById('refinedSwitchState');
      ...
      button.title = wantedFilterActive ? 'Switch refined search off' : 'Switch refined search on';
      switchState.textContent = wantedFilterActive ? 'On' : 'Off';
    ...
    function fillMapInfo(houseEl, postcode, saleText) {{
      ...
      const houseInfo = houseKey ? HOUSE_POPUP_DATA[houseKey] : null;
      const fallbackName = houseEl && houseEl.firstChild ? houseEl.firstChild.textContent.trim() : '';
      mapInfoOverlay.replaceChildren();

      const name = document.createElement('div');
      name.className = 'lot-map-card-name';
      name.textContent = (houseInfo && houseInfo.name) || fallbackName || 'Auction house';
      mapInfoOverlay.appendChild(name);

      let addressText = houseInfo && houseInfo.address ? houseInfo.address.trim() : '';
      const compactAddress = addressText.replace(/\\s+/g, '').toUpperCase();
      const compactPostcode = (postcode || '').replace(/\\s+/g, '').toUpperCase();
      if (postcode && !compactAddress.includes(compactPostcode)) {{
        addressText = addressText ? addressText + ' · ' + postcode : postcode;
      }}
      const address = document.createElement('div');
      address.className = 'lot-map-card-address';
      address.textContent = addressText || 'Address unavailable';
      mapInfoOverlay.appendChild(address);

      const sale = document.createElement('div');
      sale.className = 'lot-map-card-sale';
      sale.textContent = saleText ? 'Sale: ' + saleText : 'Sale date unavailable';
      mapInfoOverlay.appendChild(sale);
    }}
    ...
    function positionMapInfo() {{
      ...
      mapInfoOverlay.style.width = width + 'px';
      const height = mapInfoOverlay.offsetHeight;
      let left = point.x - width / 2;
      let top = point.y + 20;
      if (top + height > size.y - 12) top = point.y - height - 20;
      left = Math.max(12, Math.min(left, size.x - width - 12));
      top = Math.max(44, Math.min(top, size.y - height - 12));
      mapInfoOverlay.style.left = Math.round(left) + 'px';
      mapInfoOverlay.style.top = Math.round(top) + 'px';
    }}
    ...
    map.on('move zoom resize', positionMapInfo);
    ...
    document.querySelectorAll('.card').forEach(card => {{
      ...
      if (!pcRaw) return;
      const postcode = pcRaw.textContent.trim();
      const pc = postcode.replace(/\\s+/g, '').toUpperCase();
      const dateEl = card.querySelector('.saledate, .timeleft');
      const saleText = dateEl ? dateEl.textContent.replace(/^[📅⏱]\\s*/, '').trim() : '';
      card.addEventListener('mouseenter', () => {{
        if (houseEl) houseEl.classList.add('highlighted');
        highlightMarker(pc, houseEl, postcode, saleText);
      }});
